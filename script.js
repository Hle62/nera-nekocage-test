...
const GAS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbwiUs6R03_OEYMQ2yeusMqJSWDx1a5DEwuzoLXTC5diehVocS_Z0NOB0nhir4OXqbh-Ag/exec'; 
// ==========================================================
...
function renderItemLists() {
    ...
    // ★変更: グリッドコンテナのラッパーを追加
    stockListDiv.innerHTML = '<label>在庫補充商品</label><div class="item-grid">';
    saleListDiv.innerHTML = '<label>販売記録商品</label><div class="item-grid">';
    ...
    productList.forEach(product => {
        ...
        `;
        stockListDiv.querySelector('.item-grid').insertAdjacentHTML('beforeend', stockHtml);
        
        // 2. 販売記録リスト (sale)
        // ★変更: item-card クラスを使用し、価格表示を追加
        const saleHtml = `
        ...
        `;
        saleListDiv.querySelector('.item-grid').insertAdjacentHTML('beforeend', saleHtml);
    });
        ...
        checkbox.addEventListener('change', (e) => {
            ...
            if (controls) {
                ...
                if (e.target.checked) {
                    ...
                    if (card) card.classList.add('is-checked'); // チェック時にカードを強調
